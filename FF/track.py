...
from models import SessionModel, WeatherForecast
...
class Session:
    ...
    def save_to_db(self):
        # Supondo que você tenha um modelo SessionModel e WeatherForecast no seu banco de dados
        session_model = SessionModel.objects.create(
            airTemperature=self.airTemperature,
            trackTemperature=self.trackTemperature,
            nbLaps=self.nbLaps,
            currentLap=self.currentLap,
            # ...outros campos
        )

        for wf in self.weatherList:
            WeatherForecast.objects.create(
                time=wf.time,
                weather=wf.weather,
                trackTemp=wf.trackTemp,
                airTemp=wf.airTemp,
                rainPercentage=wf.rainPercentage,
